...
RUN pip3 install --pre --extra-index-url https://developer.download.nvidia.com/compute/redist/jp/v42 'tensorflow<2'
...
RUN curl -OL "https://github.com/protocolbuffers/protobuf/releases/download/v3.0.0/protobuf-cpp-3.0.0.tar.gz" \
...
RUN sudo pip3 install --extra-index-url https://developer.download.nvidia.com/compute/redist/jp/v42 tensorflow-gpu==1.13.1+nv19.3

# Build librealsense
RUN git clone -b v2.20.0 https://github.com/IntelRealSense/librealsense.git librealsense/
...
RUN sudo pip3 uninstall -y PyYaml catkin_tools
