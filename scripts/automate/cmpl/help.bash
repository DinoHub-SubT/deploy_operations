...
__ac_git_help() {
  local usage=(
    ...
    "About: 16... ignore   : Ignore pre-defined files from git index (catkin_profiles, pyc files)."
    "About: 17... unignore : Unignore pre-defined files from git index (catkin_profiles, pyc files)."
    "About: 18... rm       : Removes all submodules."
    "About: 19... reset    : Reset all submodules to their DETACHED HEAD state, that is committed at 1-level (i.e. reset to what the 1-level is pointing to)."
    "About: 21... clean    : Clean all submodules (second level meta repo) of outstanding changes."
    "About: 22... pull     : Pulls the 2-level latest commits from the 2-level CHECKED OUT BRANCH (i.e. 2-level must be on a branch, not DETACHED HEAD)."
    ...
    "About: 42..."
    "basestation  : 2-level meta repo: basestation local simulation or robot GUI deployment."
    "common       : 2-level meta repo: common, shared repos between all other workspaces."
    "perception   : 2-level meta repo: perception object detection and sensor drivers."
    "simuation    : 2-level meta repo: robot simulation repos such as gazebo worlds, simulated sensors drivers, etc."
    "subt_launch  : 3-level repo: shared central launch, containing shared launch files, tmux, robot params, desktop icons."
    "ugv          : 2-level meta repo: ugv local simulation or robots planning-pc, nuc, xavier, slam deployment."
    "uav          : 2-level meta repo: uav local simulation or robots ds, canary, slam deployment."
  )
  local IFS=$'\n' # split output of compgen below by lines, not spaces
  usage[0]="$(printf '%*s' "-$COLUMNS"  "${usage[0]}")"
  COMPREPLY=("${usage[@]}")
